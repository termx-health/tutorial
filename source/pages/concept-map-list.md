The concept map list can be opened from the menu or the landing page. It shows all [concept maps](page:concept-map) (map sets) available in the space.

The list provides:
- basic concept map information (id, name, source and target scope) with expandable additional details
- search by text occurrences in the concept map `id`, `name`, `title` and `description`
- the latest version and its status
- sorting by ID

Actions:
- Add a new concept map (from FHIR or FSH resources, or by URL) or duplicate an existing one.
- Delete an entire concept map.
- Click on the value in the `ID` column to open the [concept map summary](page:concept-map-summary).
- Click on the latest-version value to open the concept map version summary.
